...
// $requete = "SELECT * FROM faq WHERE id='id'";
// $result = mysqli_query($link, $requete);
// // tableau des résultats de la ligne > $data_voyageur['nom_champ']
// $data_faq = mysqli_fetch_array($result);
...
class PDF extends FPDF {
	...
	function Header() {
		...
		$this->Image('img/logo_faq.png',8,2);
		// Saut de ligne 20 mm
		$this->Ln(20);
		...
		$this->Cell(60,8,'Foire aux questions',0,1,'C',1);
		// Saut de ligne 10 mm
		$this->Ln(10);		
	}
	// Footer
...
$pdf->Cell(75,6,'DU '.$data_faq['date'],0,1,'L',1);		
$pdf->Cell(75,6,strtoupper(utf8_decode($data_faq['question'].' '.$data_faq['reponse'])),0,1,'L',1);				
...
function entete_table($position_entete) {
	...
	$pdf->SetX(10);
	$pdf->Cell(60,8,'question',1,0,'C',1);	// 60 >largeur colonne, 8 >hauteur colonne
	// position de la colonne 2 (70 = 10+60)
	$pdf->SetX(70); 
	$pdf->Cell(60,8,'reponse',1,0,'C',1);
...
$requete2 = "SELECT * FROM faq WHERE id='1'";
$result2 = mysqli_query($link, $requete2);
while ($data_visit = mysqli_fetch_array($result2)) {
	// position abcisse de la colonne 1 (10mm du bord)
	$pdf->SetY($position_detail);
	$pdf->SetX(10);
	$pdf->MultiCell(60,8,utf8_decode($data_visit['question']),1,'C');
    // position abcisse de la colonne 2 (70 = 10 + 60)	
	$pdf->SetY($position_detail);
	$pdf->SetX(70); 
	$pdf->MultiCell(60,8,utf8_decode($data_visit['reponse']),1,'C');
...
// Compteur de pages {nb}
$pdf->AliasNbPages();
$pdf->Cell(500,20,utf8_decode('fin des question'));
